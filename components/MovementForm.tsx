...
export const MovementForm: React.FC<MovementFormProps> = ({ onAdd, onUpdate, onCancel, transactions, initialData, prefill }) => {
  ...
  const [exitMode, setExitMode] = useState<'BAIXA' | 'TRANSFERENCIA'>('BAIXA');
  ...
  useEffect(() => {
    if (code) {
      const match = [...transactions].sort((a, b) => b.timestamp - a.timestamp).find(t => t.code === code);
      if (match) {
        setName(match.name);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setExitMode(initialData.destinationWarehouse ? 'TRANSFERENCIA' : 'BAIXA');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'SAIDA' && exitMode === 'TRANSFERENCIA' && originWarehouse === destWarehouse) {
    ...
    const formData = {
      ...
      destinationWarehouse: (type === 'SAIDA' && exitMode === 'TRANSFERENCIA') ? destWarehouse : undefined,
      address,
    ...
    <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-full animate-slide-up max-w-4xl mx-auto">
      <div className="flex bg-slate-100 p-1.5">
        <button
          ...
          className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${type === 'ENTRADA' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
        >
          <div className={`w-2 h-2 rounded-full ${type === 'ENTRADA' ? 'bg-emerald-500' : 'bg-slate-300'}`} />
          Entrada
        </button>
        <button
          type="button"
          onClick={() => setType('SAIDA')}
          className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${type === 'SAIDA' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
        >
          <div className={`w-2 h-2 rounded-full ${type === 'SAIDA' ? 'bg-red-500' : 'bg-slate-300'}`} />
          Saída
        </button>
      </div>
      ...
      <div className="p-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-black text-slate-800 uppercase tracking-tight text-base">
            {type === 'ENTRADA' ? 'Recebimento de Mercadoria' : 'Expedição / Baixa'}
          </h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
        ...
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Modo de Registro *</label>
          <div className="grid grid-cols-2 gap-4">
            <button
              type="button"
              onClick={() => setOpType('MOVIMENTACAO')}
              className={`p-3 rounded-xl border-2 text-left transition-all ${opType === 'MOVIMENTACAO' ? 'border-primary-600 bg-white shadow-sm' : 'border-slate-200 bg-slate-100/50'}`}
            >
              <span className={`block text-xs font-black uppercase ${opType === 'MOVIMENTACAO' ? 'text-primary-700' : 'text-slate-500'}`}>Movimentação</span>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Impacta no Saldo</p>
            </button>
            <button
              type="button"
              onClick={() => setOpType('CONTAGEM')}
              className={`p-3 rounded-xl border-2 text-left transition-all ${opType === 'CONTAGEM' ? 'border-amber-500 bg-white shadow-sm' : 'border-slate-200 bg-slate-100/50'}`}
            >
              <span className={`block text-xs font-black uppercase ${opType === 'CONTAGEM' ? 'text-amber-700' : 'text-slate-500'}`}>Apenas Contagem</span>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Auditoria de estoque</p>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Quantidade a Movimentar *</label>
        ...
        <div className="space-y-4">
          ...
          {type === 'SAIDA' && (
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Tipo de Saída</label>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <button
                  type="button"
                  onClick={() => setExitMode('BAIXA')}
                  className={`p-3 rounded-xl border-2 flex items-center gap-2 transition-all ${exitMode === 'BAIXA' ? 'border-red-500 bg-white shadow-sm' : 'border-slate-200 bg-slate-100/50'}`}
                >
                  <LogOut size={16} className={exitMode === 'BAIXA' ? 'text-red-500' : 'text-slate-400'} />
                  <span className={`text-[10px] font-black uppercase ${exitMode === 'BAIXA' ? 'text-red-700' : 'text-slate-500'}`}>Apenas Saída / Baixa</span>
                </button>
                <button
                  type="button"
                  onClick={() => setExitMode('TRANSFERENCIA')}
                  className={`p-3 rounded-xl border-2 flex items-center gap-2 transition-all ${exitMode === 'TRANSFERENCIA' ? 'border-primary-600 bg-white shadow-sm' : 'border-slate-200 bg-slate-100/50'}`}
                >
                  <ArrowRightLeft size={16} className={exitMode === 'TRANSFERENCIA' ? 'text-primary-600' : 'text-slate-400'} />
                  <span className={`text-[10px] font-black uppercase ${exitMode === 'TRANSFERENCIA' ? 'text-primary-700' : 'text-slate-500'}`}>Transferência Interna</span>
                </button>
              </div>

              {exitMode === 'TRANSFERENCIA' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-slide-down">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-black outline-none focus:border-primary-500"
                      required
                    >
                      <option value="">Selecione...</option>
                      {WAREHOUSES.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-primary-500"
                    />
                  </div>
                </div>
              )}
        ...
        <div className="pt-6 border-t border-slate-100">
          <button
            type="submit"
            className={`w-full py-4 rounded-2xl text-white font-black uppercase tracking-widest text-xs shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-3 ${type === 'ENTRADA' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}`}
          >
            <Save size={20} />
            Confirmar Registro de Kardex
          </button>
        </div>
      </form>
    </div>
