...
export const TransactionHistory: React.FC<TransactionHistoryProps> = ({ transactions, onDelete, onEdit, isRefreshing }) => {
  ...
  const filteredData = useMemo(() => {
    return transactions.filter(t => {
      ...
      if (t.quantity === 0 && t.responsible === 'SISTEMA') return false;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col h-full animate-fade-in overflow-hidden">
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/50">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight flex items-center gap-2">
              ...
              {isRefreshing && (
                <div className="flex items-center gap-1.5 px-2 py-1 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-primary-100 dark:border-primary-900/30 animate-fade-in">
                  <div className="w-2 h-2 border-2 border-primary-200 dark:border-primary-800 rounded-full animate-spin border-t-primary-600"></div>
                  <span className="text-[8px] font-black text-primary-600 dark:text-primary-400 uppercase tracking-tighter">Sincronizando...</span>
                </div>
              )}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto">
            <div className="relative flex-1 lg:w-64">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 w-full shadow-sm text-slate-900 dark:text-white"
                value={searchTerm}
            ...
            <button
              ...
              className={`p-2 rounded-lg border transition-all flex items-center gap-2 text-xs font-black uppercase tracking-tighter ${sortOrder === 'ASC' ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}
            ...
            <button
              ...
              className={`p-2 rounded-lg border transition-all flex items-center gap-2 text-xs font-bold ${showFilters ? 'bg-primary-600 border-primary-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
              <Filter size={16} />
            </button>
          </div>
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-slide-down">
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Operação</label>
              <select className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs font-bold text-slate-600 dark:text-slate-300" value={filterOp} onChange={(e) => setFilterOp(e.target.value as any)}>
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Data Final</label>
              <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs font-bold text-slate-600 dark:text-slate-300" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="overflow-y-auto flex-1 no-scrollbar p-2 md:p-0">
        <table className="w-full hidden md:table border-collapse">
          <thead className="bg-slate-50 dark:bg-slate-800/50 sticky top-0 z-10 border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th className="px-6 py-4 text-left text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Produto - SKU</th>
              ...
              <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                <td className="px-6 py-4">
                  <div className="flex items-center">
                    <div className="flex-shrink-0 h-10 w-10 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center border border-slate-200 dark:border-slate-700 overflow-hidden">
                      {(() => {
                    ...
                    <div className="ml-4">
                      <div className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight">{t.name}</div>
                      <div className="text-[10px] font-mono text-slate-500 dark:text-slate-400 uppercase">{t.code}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-xs font-bold text-slate-700 dark:text-slate-300">
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="bg-slate-800 dark:bg-slate-700 text-white text-[9px] px-1.5 py-0.5 rounded">ORIGEM: {t.warehouse}</span>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] font-black text-primary-700 dark:text-primary-400 uppercase">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => onEdit(t)} className="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-lg"><Pencil size={18} /></button>
                    <button onClick={() => confirm('Excluir?') && onDelete(t.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg"><Trash2 size={18} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="md:hidden flex flex-col gap-3 py-2 pb-24">
          {filteredData.map((t) => (
            <div key={t.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm active:bg-slate-50 dark:active:bg-slate-800 transition-colors">
              <div className="p-4 flex items-center justify-between border-b border-slate-50 dark:border-slate-800">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-slate-50 dark:bg-slate-800 rounded-lg flex items-center justify-center border border-slate-100 dark:border-slate-700 text-slate-400 overflow-hidden">
                    {(() => {
                      const photo = t.photos?.[0] || transactions.find(tx => tx.code === t.code && tx.photos?.length > 0)?.photos?.[0];
                  ...
                  <div>
                    <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase leading-none mb-1">{t.name}</h4>
                    <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500 tracking-tighter uppercase">{t.code}</span>
                  </div>
                </div>
              ...
              <div className="p-4 bg-slate-50/50 flex flex-col gap-3">
                <div className="space-y-1">
                  <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Fluxo Logístico</p>
                  <div className="flex items-center gap-2 text-[10px] font-bold">
                    <span className="bg-slate-700 dark:bg-slate-800 text-white px-1.5 py-0.5 rounded text-[8px]">DE: {t.warehouse}</span>
                    {t.destinationWarehouse && (
                      <>
                ...
                <div className="grid grid-cols-2 gap-2 border-t border-slate-100 dark:border-slate-800 pt-2">
                  <div>
                    <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase mb-1">Registro</p>
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-600 dark:text-slate-400">
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <button onClick={() => onEdit(t)} className="p-2 text-primary-600 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm"><Pencil size={14} /></button>
                    <button onClick={() => confirm('Excluir?') && onDelete(t.id)} className="p-2 text-red-600 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm"><Trash2 size={14} /></button>
                  </div>
                </div>
              </div>
            </div>
